...
// ---------------------------------------------------------------------------
// Unified event attachment helpers (delegates to clientEvents.js)
// ---------------------------------------------------------------------------

const unifiedAppendMessage = async (session, worktreeId, message) => {
  await appendWorktreeMessage(session, worktreeId, message);
};

const unifiedBroadcastDiff = async (sessionId, worktreeId) => {
  if (!worktreeId || worktreeId === "main") {
    await broadcastRepoDiff(sessionId);
  } else {
    await broadcastWorktreeDiff(sessionId, worktreeId);
  }
};

const buildEventDeps = () => ({
  getSession,
  broadcastToSession,
